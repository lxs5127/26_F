...
void Interrupt_Init(void)
{
    ...
    NVIC_EnableIRQ(TIMER_controll_INST_INT_IRQN);
    NVIC_EnableIRQ(PWM_0_INST_INT_IRQN);
    NVIC_EnableIRQ(GPIO_MULTIPLE_GPIOB_INT_IRQN);
    NVIC_EnableIRQ(I2C_OLED_INST_INT_IRQN);
    NVIC_EnableIRQ(TIMER_read_INST_INT_IRQN);
    NVIC_EnableIRQ(TIMER_tepper_INST_INT_IRQN);
    NVIC_EnableIRQ(GPIO_KEY_GPIOA_INT_IRQN);
    ...
    switch (DL_Interrupt_getPendingGroup(DL_INTERRUPT_GROUP_1)) {
        case GPIO_MULTIPLE_GPIOB_INT_IIDX:
        {
            uint32_t pending = DL_GPIO_getEnabledInterruptStatus(GPIOB,
                GPIO_encoder_PIN_encoder_left_A_PIN | GPIO_encoder_PIN_encoder_left_B_PIN |
                GPIO_encoder_PIN_encoder_right_A_PIN | GPIO_encoder_PIN_encoder_right_B_PIN |
                GPIO_MPU6050_PIN_INT_PIN);

            if (pending & (GPIO_encoder_PIN_encoder_left_A_PIN | GPIO_encoder_PIN_encoder_left_B_PIN |
                           GPIO_encoder_PIN_encoder_right_A_PIN | GPIO_encoder_PIN_encoder_right_B_PIN))
                encoder_read(pending);
